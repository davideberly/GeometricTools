...
LONG constexpr gWindowWidth = 1024;
LONG constexpr gWindowHeight = 256;

static bool CreateHeaderFile(std::wstring const& fontName, int weight, int size, DWORD italic)
{
    std::size_t constexpr maxCharacters = 1024;
    ...
    fwprintf(output, L"// Copyright (c) 1998-2026\n");
    ...
    fwprintf(output, L"// Version: autogenerated\n");
    fwprintf(output, L"\n");
...
static bool CreateSourceFile(std::wstring const& fontName, int weight, int size,
...
static void CreateFontData(HDC hDC, int& width, int& height, std::vector<unsigned char>& texels,
    float characterData[257])
{
    // Get the height of the font.
    TEXTMETRIC metric{};
    GetTextMetrics(hDC, &metric);
    height = static_cast<int>(metric.tmHeight);
    ...
    texels.resize(static_cast<std::size_t>(width) * static_cast<std::size_t>(height));
    ...
    for (int i = 0; i < 256; ++i)
    {
        ...
        for (int row = 0; row < height; ++row)
        {
            for (int col = 0; col < charWidth; ++ col)
            {
                ...
                texels[static_cast<std::size_t>(width) * row + start + col] = GetRValue(kColor);
            }
        }
...
static void ProcessFont(HWND hWnd, std::wstring const& fontName, int weight,
    int size, DWORD italic)
{
    ...
    TextOut(hDC, 8, 3 * size, msg, static_cast<int>(wcslen(msg)));
