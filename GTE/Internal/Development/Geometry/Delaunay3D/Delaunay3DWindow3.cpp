...
bool Delaunay3DWindow3::CreateScene()
{
    ...
    std::ifstream input("C:\\Users\\deber\\GeometricTools\\GTE\\Internal\\Development\\Geometry\\Delaunay3D\\sandeep.txt");
    int numVertices = 3066;
    ...
    ComputeExtremes(numVertices, mVertices.data(), pmin, pmax);
    Vector3<float> center = 0.5f * (pmin + pmax);
    Vector3<float> extreme = 0.5f * (pmax - pmin);
