...
namespace gte
{
    ...
    public:
            ...
            PolynomialRoot<T>* roots)
        {
            ...
            if (g4 == zero)
            {
                return RootsCubic<T>::Solve(g0, g1, g2, g3, roots);
            }

            // Test for zero-valued roots.
            if (g0 == zero)
            {
                return HasZeroValuedRoots(g1, g2, g3, g4, roots);
            }
            ...
            size_t numRoots = ComputeDepressedRoots(rD0, rD1, rD2, rRoots.data());
            ...
            PolynomialRoot<T>* roots)
        {
            ...
            if (m0 == zero)
            {
                return HasZeroValuedRoots(m1, m2, m3, roots);
            }
            ...
            PolynomialRoot<T>* roots)
        {
            ...
            std::array<PolynomialRoot<Rational>, 4> rRoots{};
            size_t numRoots = ComputeDepressedRoots(d0, d1, d2, rRoots.data());
            for (size_t i = 0; i < numRoots; ++i)
            {
            ...
            PolynomialRoot<Rational>* rRoots)
        {
            ...
            if (signD0 == 0)
            {
                if (signD1 == 0)
                {
                    ...
                    else if (signD2 < 0)
                    {
                        ...
                        RootsQuadratic<Rational>::ComputeDepressedRoots(rD2, rQRoots.data());
                ...
                else
                {
                    ...
                    size_t numRoots = RootsCubic<Rational>::ComputeDepressedRoots(rD1, rD2, rRoots);
            ...
            if (signD1 == 0)
            {
                return SolveBiquadratic(rD0, rD2, rRoots);
            }
            ...
            if (signDelta == 0)
            {
                ...
                if (rQDiscriminant.GetSign() > 0)
                {
                    ...
                    std::array<PolynomialRoot<Rational>, 2> rQRoots{};
                    RootsQuadratic<Rational>::ComputeDepressedRoots(-rQDiscriminant, rQRoots.data());
    ...
    private:
        ...
        static size_t HasZeroValuedRoots(T const& g1, T const& g2, T const& g3, T const& g4,
            PolynomialRoot<T>* roots)
        {
            T const zero = static_cast<T>(0);
            if (g1 == zero)
            {
                ...
                {
                    size_t numRoots = RootsQuadratic<T>::Solve(g2, g3, g4, roots);
            ...
            {
                size_t numRoots = RootsCubic<T>::Solve(g1, g2, g3, g4, roots);
        ...
        static size_t HasZeroValuedRoots(T const& m1, T const& m2, T const& m3,
            PolynomialRoot<T>* roots)
        {
            T const zero = static_cast<T>(0);
            if (m1 == zero)
            {
                ...
                {
                    size_t numRoots = RootsQuadratic<T>::Solve(m2, m3, roots);
                    roots[numRoots++] = { zero, 2 };
                    std::sort(roots, roots + numRoots);
                    return numRoots;
                }
            ...
            {
                size_t numRoots = RootsCubic<T>::Solve(m1, m2, m3, roots);
                roots[numRoots++] = { zero, 1 };
                std::sort(roots, roots + numRoots);
                return numRoots;
            }
        ...
        static size_t SolveBiquadratic(
            ...
            PolynomialRoot<Rational>* rRoots)
        {
            ...
            {
                std::array<PolynomialRoot<Rational>, 2> rQRoots{};
                RootsQuadratic<Rational>::ComputeDepressedRoots(-rT, rQRoots.data());
                ...
                {
                    // Four real roots.
                    RootsQuadratic<Rational>::ComputeDepressedRoots(-rSMsqrtT, rQRoots.data());
                    Rational r0 = rQRoots[1].x;
                    RootsQuadratic<Rational>::ComputeDepressedRoots(-rSPsqrtT, rQRoots.data());
            ...
            else
            {
                if (rS.GetSign() > 0)
                {
                    ...
                    std::array<PolynomialRoot<Rational>, 2> rQRoots{};
                    RootsQuadratic<Rational>::ComputeDepressedRoots(-rS, rQRoots.data());
                    Rational r0 = rQRoots[1].x;
                    if (r0.GetSign() > 0)
                    {
                        rRoots[0] = { -r0, 2 };
